...
import argparse
import torch
from multi_components.generators.multi_task_generators import get_data_weight_output_path
...
def analyzer_process(method:str='gradcam', target_category=None, model_flag:str='resnet', task:str='ddsm', dataset_split:str='val'):
...
def evaluator_process(method:str='gradcam', target_category=None, model_flag:str='resnet', task:str='CatsDogs', dataset_split:str='val',
    ...
    for i in range(0, 1):
        ...
        "xgradcam":[XGradCAM_A, XGradCAM_P]}
        cam_method = cam_method_zoo[general_args.method]
        evaluator_main(cam_method[1], model, target_layer, val_dataset, general_args, im_path, version=version)



if __name__ == '__main__':
    task = 'siim' # 'luna', 'rsna', 'siim', 'us' # , 'ddsm' 
    model_flag = 'resnet'  # doesn't matter, not used
    methods = ['gradcam', 'fullcam', 'gradcampp', 'xgradcam']
    ds = 'train'  #  ['val', 'train']

    for method in methods:
        analyzer_process(method=method, target_category=None, model_flag=model_flag, task=task, dataset_split=ds)
    ...
    #                 #                     max_iter=None, mm_ratio=1.0
    #                 #                    )
    #                 evaluator_process(method=method, target_category=1, model_flag=model_flag, task=task, dataset_split='val',
    #                                     randomization=False, random_severity=0,
    #                                     maxmin_flag=mm, remove_minus_flag=rm, im_selection_mode=im, im_selection_extra=0.3,
    #                                     max_iter=None, version='corr'
    #                                     )
